...
SDL_Surface *SDL_ResizeXY(SDL_Surface *image, int new_w, int new_h, int filter){
	...
	if (SDL_MUSTLOCK(dest)== SDL_TRUE) SDL_LockSurface(dest);
	alpha=0; r=0; g=0; b=0;
	usealpha=0;
	cx=0;
	for(; cx < dest->w; cx++){ //check the whole image for any occurance of alpha
		int cy=0;
		for(; cy < dest->h; cy++){
			SDL_GetRGBA(filter_GetPixel(dest, cx, cy), dest->format, &r, &g, &b, &alpha);
			if(alpha != SDL_ALPHA_OPAQUE){
				usealpha=1; cx=dest->w; break;
			}
		}
	}
	if (SDL_MUSTLOCK(dest)== SDL_TRUE) SDL_UnlockSurface(dest);
...
static SDL_Surface *filter_resizexy(SDL_Surface *source, int new_w, int new_h, int filter){
	...
	if (SDL_MUSTLOCK(source)== SDL_TRUE) SDL_LockSurface(source);
	if (SDL_MUSTLOCK(dest)== SDL_TRUE)   SDL_LockSurface(dest);

	filter_zoom2(dest, source, f, s);

	if (SDL_MUSTLOCK(dest)== SDL_TRUE)   SDL_UnlockSurface(dest);
	if (SDL_MUSTLOCK(source)== SDL_TRUE) SDL_UnlockSurface(source);
